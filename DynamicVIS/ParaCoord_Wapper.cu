...
extern "C"
void cuda_CleanConstant()
{
	cudaFree(d_item_offset_ptr);
	cudaFree(d_item_bytes_ptr);
	cudaFree(d_item_values_ptr);
	cudaFree(d_buffer_ptr);

	d_item_offset_ptr = NULL;
	d_item_bytes_ptr = NULL;
	d_item_values_ptr = NULL;
	d_buffer_ptr = NULL;
}

extern "C"
void cuda_TransferConstant(unsigned int *_item_desc_offset,	unsigned int *_item_desc_bytes,
...
extern "C"
void cuda_TransferDynamic(  unsigned int* _filterData,			unsigned int _filterSize,
...
{
	//copy to the CPU
	checkCudaErrors(cudaMemcpy(h_compactList,d_compactValue_ptr,sizeof(unsigned int)*dn_selectItemsNum*dn_compactItemNum,cudaMemcpyDeviceToHost));
}
...
void cuda_GetCompactListSize(unsigned int &_compactNum)
{
	...
	cudaDeviceSynchronize();
	CLogFile::GetLogFile()->Add("Step-1 Get dummy Value",timer.GetTimeElapsed()*1000.0f);
	timer.StartTimer();
 
	//step 2 sort the dummy value;
	thrust::sort_by_key(d_dummyValue, d_dummyValue+dn_filterDataNum, d_dummyIndex);
	cudaDeviceSynchronize();
	CLogFile::GetLogFile()->Add("Step-2 Sort Dummy Value",timer.GetTimeElapsed()*1000.0f);
	timer.StartTimer();
	...
	cudaDeviceSynchronize();
	CLogFile::GetLogFile()->Add("Step-3 Minus Dummy Value",timer.GetTimeElapsed()*1000.0f);
	timer.StartTimer();

	//step 4 scan the dummy value
	thrust::exclusive_scan(d_dummyBool, d_dummyBool+dn_filterDataNum,d_dummySum);
	cudaDeviceSynchronize();
	CLogFile::GetLogFile()->Add("Step-4 Scan Dummy Value",timer.GetTimeElapsed()*1000.0f);
	timer.StartTimer();
	...
	gpu_compactValue<<< block_num, BLOCK_SIZE >>>(d_buffer_ptr,			d_item_offset_ptr,
												  ...
												  dn_compactItemNum);
	cudaDeviceSynchronize();
	CLogFile::GetLogFile()->Add("Step-5 Get CompactValue and Size",timer.GetTimeElapsed()*1000.0f);
	timer.StartTimer();
...
void cuda_GetVertexIndexSize(unsigned int &vertexBufferSize,	unsigned int &edgeBufferSize)
{
	...
	cudaDeviceSynchronize();
	CLogFile::GetLogFile()->Add("Step-7 Sort compactList",timer.GetTimeElapsed()*1000.0f);
	timer.StartTimer();
	...
	cudaDeviceSynchronize();
	CLogFile::GetLogFile()->Add("Step-8 GetBoolVertex",timer.GetTimeElapsed()*1000.0f);
	timer.StartTimer();
	...
	CLogFile::GetLogFile()->Add("Step-9 Exclusive Scan vertex",timer.GetTimeElapsed()*1000.0f);
	timer.StartTimer();
	...
	cudaDeviceSynchronize();
	CLogFile::GetLogFile()->Add("Step-10 EdgeDummyValue",timer.GetTimeElapsed()*1000.0f);
	timer.StartTimer();
	...
	CLogFile::GetLogFile()->Add("Step-11 Sort EdgeDummyValue",timer.GetTimeElapsed()*1000.0f);
	timer.StartTimer();
	...
	gpu_edgeBool<<< block_num, BLOCK_SIZE >>>(d_edgeDummy_ptr,		d_edgeBool_ptr,
											  dn_edgeNum,			dn_compactItemNum);
	cudaDeviceSynchronize();
	CLogFile::GetLogFile()->Add("Step-12 EdgeBoolValue",timer.GetTimeElapsed()*1000.0f);
	timer.StartTimer();
	...
	CLogFile::GetLogFile()->Add("Step-13 GetIndexBufferSize",timer.GetTimeElapsed()*1000.0f);
	timer.StartTimer();
								...
								unsigned int depth)
{
	...
	gpu_generateVertexBuffer<<< block_num, BLOCK_SIZE >>>(d_vertex_bool_ptr,		d_vertex_prefix_ptr,
														  ...
														  depth);
	cudaDeviceSynchronize();
	CLogFile::GetLogFile()->Add("Step-14 GenerateVertexBuffer",timer.GetTimeElapsed()*1000.0f);
	timer.StartTimer();
	...
	fprintf(stdout, "\n vertex index %d\n",dn_vertexNum);
	fprintf(stdout, "hello world\n");
	thrust::host_vector<float> H33(vertexBufferSize,0.0f);
	thrust::device_ptr<float> d_buffer(d_vertexBuffer_ptr);
	thrust::copy(d_buffer, d_buffer+vertexBufferSize, H33.begin());
	for(int i = 0; i<vertexBufferSize; i++)
		fprintf(stdout,"%f	",H33[i]);
	fprintf(stdout,"\n");
#endif
	...
	gpu_generateEdgeBuffer<<< block_num, BLOCK_SIZE >>>(d_vertex_Index_ptr,		d_edgeBuffer_ptr,
														...
														dn_maxDimension);
	cudaDeviceSynchronize();
	CLogFile::GetLogFile()->Add("Step-15 GenerateIndexBuffer",timer.GetTimeElapsed()*1000.0f);
	timer.StartTimer();

#ifdef DEBUG_MODE
	fprintf(stdout, "\nhello world\n");
	thrust::device_ptr<unsigned int> d_edgeBuffer(d_edgeBuffer_ptr);
	thrust::host_vector<uchar> H2(edgeBufferSize,0);
	thrust::copy(d_edgeBuffer,d_edgeBuffer+edgeBufferSize,H2.begin());
	fprintf(stdout, "\nhello world\n");
	for(uint i=0; i<edgeBufferSize; i++)
